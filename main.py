...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    Use `matplotlib` to generate a figure of the distance & #expanded-nodes
     w.r.t. the weight.
    DONE [Ex.15]: Complete the implementation of this method.
    """
    ...
    # Done: Set its label to be 'Solution cost'.
    ...
    # DONE: Set its label to be '#Expanded states'.
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    for weight in weights:
        astar_algorithm = AStar(
            heuristic_function_type=heuristic_type,
            heuristic_weight=weight,
            max_nr_states_to_expand=max_nr_states_to_expand
        )
        res = astar_algorithm.solve_problem(problem)
    ...
    plot_distance_and_expanded_wrt_weight_figure(
        problem_name="astar weights effect demo",
        weights=solution_weights,
        total_cost=costs,
        total_nr_expanded=num_expanded_states
    )
...
def toy_map_problem_experiments():
    ...
    run_astar_for_weights_in_range(heuristic_type=AirDistHeuristic, problem=toy_map_problem)
...
def basic_mda_problem_experiments():
    ...
    # Ex.18
    # DONE: create an instance of `UniformCost`, solve the `small_mda_problem_with_distance_cost`
    #       with it and print the results.
    uc = UniformCost()
    solution = uc.solve_problem(small_mda_problem_with_distance_cost)
    print(solution)
...
def mda_problem_with_astar_experiments():
    ...
    # Ex.22
    # DONE: create an instance of `AStar` with the `MDAMaxAirDistHeuristic`,
    ...
    # Ex.25
    # DONE: create an instance of `AStar` with the `MDASumAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    ...
    # Ex.28
    # DONE: create an instance of `AStar` with the `MDAMSTAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
...
def mda_problem_with_weighted_astar_experiments():
    ...
    # AKIVA: I fixed the function call to run over moderate_mda_problem_with_distance_cost
    # instead of small_mda_problem_with_distance_cost
...
def multiple_objectives_mda_problem_experiments():
    ...
    MST_air_dist_AStar = AStar(heuristic_function_type=MDAMSTAirDistHeuristic)
    ...
    optimal_distance_cost = MST_air_dist_AStar.solve_problem(moderate_mda_problem_with_distance_cost).solution_g_cost
    ...
    #          elsewhere next.
    tests_travel_dist_to_nearest_lab_A2_variant_AStar = AStar(
        heuristic_function_type=MDATestsTravelDistToNearestLabHeuristic,
